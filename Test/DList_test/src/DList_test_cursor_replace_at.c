/**
 @file DList_test_cursor_replace_at.c
 @author Greg Lee
 @version 2.0.0
...
void test_cursor_replace_at_1( void )
{
   ...
   i_dlist_cursor_replace_at( cursor, 25 );

   CU_ASSERT( i_dlist_cursor_item_at( cursor ) == 25 );
...
void test_cursor_replace_at_2( void )
{
   ...
   i_dlist_cursor_replace_at( cursor, 8 );
   
   i_dlist_cursor_back( cursor );
   
   i_dlist_cursor_replace_at( cursor, 14 );
   ...
   CU_ASSERT( i_dlist_cursor_item_at( cursor ) == 14 );

   i_dlist_cursor_forth( cursor );

   CU_ASSERT( i_dlist_cursor_item_at( cursor ) == 8 );
...
void test_cursor_replace_at_3( void )
{
   ...
   s_dlist_cursor_replace_at( cursor, s6 );
   
   s_dlist_cursor_back( cursor );
   
   s_dlist_cursor_replace_at( cursor, s5 );
   
   s_dlist_cursor_back( cursor );
   
   s_dlist_cursor_replace_at( cursor, s4 );

   s_dlist_cursor_start( cursor );
   
   CU_ASSERT( s_dlist_cursor_item_at( cursor ) == s4 );

   s_dlist_cursor_forth( cursor );

   CU_ASSERT( s_dlist_cursor_item_at( cursor ) == s5 );

   s_dlist_cursor_forth( cursor );

   CU_ASSERT( s_dlist_cursor_item_at( cursor ) == s6 );
