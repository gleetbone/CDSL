...
void test_edge_set_1( void )
{
   ...
   ii_dgraph_edge_set( dgraph, v_from, v_to, 31 );
   value = ii_dgraph_edge_value_from_to( dgraph, v_from, v_to );
   CU_ASSERT( value == 31 );
...
add_test_edge_set( void )
{
   ...
   p_suite = CU_add_suite("suite_test_edge_set", NULL, NULL);
   if (NULL == p_suite)
   {
      CU_cleanup_registry();
      return CU_get_error();
   ...
   // test_edge_set_1
   add_test_to_suite( p_suite, test_edge_set_1, "test_edge_set_1" );
