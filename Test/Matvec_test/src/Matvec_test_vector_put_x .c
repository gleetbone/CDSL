/**
 @file Matvec_test_vector_put_x.c
 @author Greg Lee
 @version 1.0.0
...
void test_vector_put_x_1( void )
{
   f_matvec_t *fm = NULL;

   fm = f_matvec_make( 1, 3 );

   f_matvec_vector_put_x( fm, 1.0 );
...
void test_vector_put_x_2( void )
{
   d_matvec_t *dm = NULL;

   dm = d_matvec_make( 1, 3 );

   d_matvec_vector_put_x( dm, 1.0 );
...
void test_vector_put_x_3( void )
{
   cf_matvec_t *cfm = NULL;

   cfm = cf_matvec_make( 1, 3 );

   cf_matvec_vector_put_x( cfm, 1.0 + 1.0*I );
...
void test_vector_put_x_4( void )
{
   cd_matvec_t *cdm = NULL;

   cdm = cd_matvec_make( 1, 3 );

   cd_matvec_vector_put_x( cdm, 1.0 + 1.0*I );
...
add_test_vector_put_x( void )
{
   ...
   p_suite = CU_add_suite("suite_test_vector_put_x", NULL, NULL);
   if (NULL == p_suite)
   {
      CU_cleanup_registry();
      return CU_get_error();
   ...
   // test_vector_put_x_1
   add_test_to_suite( p_suite, test_vector_put_x_1, "test_vector_put_x_1" );

   // test_vector_put_x_2
   add_test_to_suite( p_suite, test_vector_put_x_2, "test_vector_put_x_2" );

   // test_vector_put_x_3
   add_test_to_suite( p_suite, test_vector_put_x_3, "test_vector_put_x_3" );

   // test_vector_put_x_4
   add_test_to_suite( p_suite, test_vector_put_x_4, "test_vector_put_x_4" );
