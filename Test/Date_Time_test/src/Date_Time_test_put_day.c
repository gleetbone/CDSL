...
add_test_put_day( void )
{
   ...
   p_suite = CU_add_suite("suite_test_put_day", NULL, NULL);
   if (NULL == p_suite) 
   {
      CU_cleanup_registry();
      return CU_get_error();
   ...
   // test_put_day
   add_test_to_suite( p_suite, test_put_day, "test_put_day" );
