/**
 @file Queue_test_is_deep_equal.c
 @author Greg Lee
 @version 2.0.0
 @brief: "tests for Queue_deep_equal"
 ...
 Unit tests for Queue_deep_equal.
...
void test_is_deep_equal_3( void )
{
   ...
   CU_ASSERT( int_queue_is_deep_equal( list, list1 ) == 0 );

   int_queue_put( list, 23 );
...
{
   s_queue_t *list = NULL;
   s_queue_t *list1 = NULL;
   s_queue_t *list2 = NULL;
   ...
   s_queue_put( list2, s2 );

   CU_ASSERT( s_queue_is_deep_equal( list, list1 ) == 1 );
   CU_ASSERT( s_queue_is_deep_equal( list, list2 ) == 1 );

   CU_ASSERT( s_queue_is_deep_equal( list, list1 ) == 1 );
   CU_ASSERT( s_queue_is_deep_equal( list, list2 ) == 1 );

   s_queue_deep_dispose( &list );
   s_queue_dispose( &list1 );
   s_queue_deep_dispose( &list2 );
