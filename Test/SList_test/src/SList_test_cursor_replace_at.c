/**
 @file SList_test_cursor_replace_at.c
 @author Greg Lee
 @version 2.0.0
...
void test_cursor_replace_at_1( void )
{
   ...
   int_slist_cursor_replace_at( cursor, 25 );

   CU_ASSERT( int_slist_cursor_item_at( cursor ) == 25 );
...
void test_cursor_replace_at_2( void )
{
   ...
   int_slist_cursor_go( cursor, 2 );

   int_slist_cursor_replace_at( cursor, 8 );

   int_slist_cursor_go( cursor, 1 );

   int_slist_cursor_replace_at( cursor, 14 );

   int_slist_cursor_go( cursor, 0 );
   ...
   CU_ASSERT( int_slist_cursor_item_at( cursor ) == 14 );

   int_slist_cursor_forth( cursor );

   CU_ASSERT( int_slist_cursor_item_at( cursor ) == 8 );
...
void test_cursor_replace_at_3( void )
{
   ...
   s_slist_cursor_go( cursor, 2 );

   s_slist_cursor_replace_at( cursor, s6 );

   s_slist_cursor_go( cursor, 1 );

   s_slist_cursor_replace_at( cursor, s5 );

   s_slist_cursor_go( cursor, 0 );

   s_slist_cursor_replace_at( cursor, s4 );

   s_slist_cursor_start( cursor );

   CU_ASSERT( s_slist_cursor_item_at( cursor ) == s4 );

   s_slist_cursor_forth( cursor );

   CU_ASSERT( s_slist_cursor_item_at( cursor ) == s5 );

   s_slist_cursor_forth( cursor );

   CU_ASSERT( s_slist_cursor_item_at( cursor ) == s6 );
...
add_test_cursor_replace_at( void )
{
   ...
   p_suite = CU_add_suite("suite_test_cursor_replace_at", NULL, NULL);
   if (NULL == p_suite)
   {
      CU_cleanup_registry();
      return CU_get_error();
   ...
   // test_cursor_replace_at_1
   add_test_to_suite( p_suite, test_cursor_replace_at_1, "test_cursor_replace_at_1" );
