/**
 @file Heap_test_is_deep_equal.c
 @author Greg Lee
 @version 2.0.0
...
void test_is_deep_equal_1( void )
{
   ...
   CU_ASSERT( i_heap_is_deep_equal( heap, heap ) == 1 );
   
   heap1 = i_heap_make();
   
   CU_ASSERT( i_heap_is_deep_equal( heap1, heap ) == 0 );
...
void test_is_deep_equal_2( void )
{
   ...
   s2 = string_make_from_cstring( "13" );
   ...
   CU_ASSERT( s_heap_is_equal( heap1, heap ) == 0 );
   CU_ASSERT( s_heap_is_deep_equal( heap1, heap ) == 1 );

   s_heap_deep_dispose( &heap );
   s_heap_deep_dispose( &heap1 );
