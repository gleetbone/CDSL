/**
 @file BTree_kv_test_cursor_index.c
 @author Greg Lee
 @version 2.0.0
 @brief: "tests for BTree_kv_cursor_index"
...
void test_cursor_index_1( void )
{
   ...
   ii_btree_kv_cursor_t *cursor = ii_btree_kv_cursor_make( btree );

   CU_ASSERT( ii_btree_kv_cursor_off( cursor ) == 1 );
   CU_ASSERT( ii_btree_kv_cursor_index( cursor ) == -1 );

   ii_btree_kv_put( btree, 240, 24 );
   ii_btree_kv_put( btree, 130, 13 );

   ii_btree_kv_cursor_start( cursor );
   CU_ASSERT( ii_btree_kv_cursor_index( cursor ) == 0 );
...
void test_cursor_index_2( void )
{
   ...
   ss_btree_kv_cursor_t *cursor = ss_btree_kv_cursor_make( btree );

   CU_ASSERT( ss_btree_kv_cursor_off( cursor ) == 1 );
   CU_ASSERT( ss_btree_kv_cursor_index( cursor ) == -1 );

   ss_btree_kv_put( btree, s1v, s1k );
   ss_btree_kv_put( btree, s2v, s2k );

   ss_btree_kv_cursor_start( cursor );
   CU_ASSERT( ss_btree_kv_cursor_index( cursor ) == 0 );
   ...
   ss_btree_kv_cursor_dispose( &cursor );
