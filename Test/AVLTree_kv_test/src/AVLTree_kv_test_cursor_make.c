...
void test_cursor_make_3( void )
{
   ...
   ss_avltree_kv_cursor_t *cursor1 = NULL;
   ss_avltree_kv_cursor_t *cursor2 = NULL;

   avltree = ss_avltree_kv_make();
   cursor1 = ss_avltree_kv_cursor_make( avltree );
   cursor2 = ss_avltree_kv_cursor_make( avltree );

   CU_ASSERT( cursor1 != NULL );
   CU_ASSERT( cursor2 != NULL );

   ss_avltree_kv_cursor_dispose( &cursor2 );
   ss_avltree_kv_cursor_dispose( &cursor1 );
