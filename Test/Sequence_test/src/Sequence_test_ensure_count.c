...
void test_ensure_count_3( void )
{
   s_sequence_t *sequence = NULL;

   sequence = s_sequence_make_n( 12 );

   s_sequence_set_count( sequence, 1 );
   
   CU_ASSERT( s_sequence_count( sequence ) == 1 );
   CU_ASSERT( s_sequence_capacity( sequence ) == 12 );
   
   s_sequence_ensure_count( sequence, 4 );
...
add_test_ensure_count( void )
{
   ...
   // test_ensure_count_3
   add_test_to_suite( p_suite, test_ensure_count_3, "test_ensure_count_3" );
