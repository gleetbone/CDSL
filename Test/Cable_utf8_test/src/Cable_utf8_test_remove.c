...
void test_remove_2( void )
{
   ...
   cable_utf8_remove( cable, 1 );

   CU_ASSERT( cable_utf8_is_equal_cstring( cable, "acd" ) == 1 );
...
void test_remove_3( void )
{
   ...
   cable_utf8_remove( cable, 2 );

   CU_ASSERT( cable_utf8_is_equal_cstring( cable, "abd" ) == 1 );
...
void test_remove_6( void )
{
   ...
   cable_utf8_remove( cable, 5 );

   CU_ASSERT( cable_utf8_is_equal_cstring( cable, "abcdeghij" ) == 1 );
...
void test_remove_7( void )
{
   ...
   cable_utf8_remove( cable, 6 );

   CU_ASSERT( cable_utf8_is_equal_cstring( cable, "abcdefhij" ) == 1 );
...
void test_remove_9( void )
{
   ...
   for( k=0; k<10000; k++ )
   {
      ...
      {
         cp = random_cstring();
         start_index = rand() % ( cable_utf8_count( cable ) + 1 );
         cable_utf8_insert_cstring( cable, cp, start_index );
         free( cp );
         i = i + 1;
      }

      j = 0;
      while ( ( j < count/4 ) && ( cable_utf8_count( cable ) > 0 ) )
      {
         start_index = rand() % ( cable_utf8_count( cable ) );

         if ( start_index < ( cable_utf8_count( cable ) - 1 ) )
         {
            end_index = start_index + ( rand() % ( cable_utf8_count( cable ) - start_index ) );
         }
         else
         {
         ...
         cable_utf8_remove_substring( cable, start_index, end_index );
         j = j + 1;
      }
